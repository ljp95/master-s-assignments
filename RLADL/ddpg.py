...
class DDPG(nn.Module):
    def __init__(self,input_size,nb_features,action_space,gamma=0.99,device='cpu',criterion=nn.MSELoss()):
        ...
        self.critic = NN_bn(input_size+action_space.shape[0],action_space.shape[0],nb_features)
        self.new_critic = NN_bn(input_size+action_space.shape[0],action_space.shape[0],nb_features)
...
def update(model,buffer,batch_size):
    ...
    q_values_noisy = model.critic(torch.cat((batch_features,batch_action),dim=1))
    ...
    q_values = model.critic(torch.cat((batch_features,model.actor(batch_features)),dim=1))
