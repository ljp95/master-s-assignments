...
class ActorCritic(nn.Module):
    def __init__(self,featExtractor,nb_features,action_space,gamma=0.99,device='cpu',beta=1,kl_target=0.01,K=5,update_timestep=2000):
        ...
        self.actor_optimizer = optim.Adam(self.new_actor.parameters(),lr=1e-4)
        self.critic_optimizer = optim.Adam(self.critic.parameters(),lr=1e-3)
        self.timestep = 0
        self.update_timestep = update_timestep
        self.beta = beta
        
    def forward(self,features,random=False):
...
def play_episode(model,env,T=200,random=True):   
    ...
    for t in range(T):
        ...
        action = model(features,random=random)
...
def update(model,criterion):
    ...
    for s in range(model.K):        
        ...
        surrogate_loss = -torch.mean(ratios*advantages)
        values_loss = criterion(values.view(-1),gt_returns)
        kl_divergence = nn.KLDivLoss(reduction='batchmean',log_target=True)(log_probas,old_actions_probas)

        loss = surrogate_loss + values_loss + kl_divergence*model.beta
    ...
    if kl_divergence >= 1.5*model.kl_target:
        model.beta *= 2
    if kl_divergence <= model.kl_target/1.5:
        model.beta *= 0.5
