                    ...
                    const Image<byte>& im2, int i2,int j2,float m2)
{
    ...
    for(int dx=-win;dx<=win;dx++){
        for(int dy=-win;dy<=win;dy++){
            float tmp1 = im1(i1+dx,j1+dy)-m1;
            float tmp2 = im2(i2+dx,j2+dy)-m2;
            dist += tmp1*tmp2;
            var1 += tmp1*tmp1;
            var2 += tmp2*tmp2;
        }
...
static void find_seeds(Image<byte> im1, Image<byte> im2,
    ...
    for(int y=win; y+win<maxy; y++) {
        ...
        for(int x=win; x+win<im1.width(); x++) {
            ...
            Seed q = Seed(x,y,dMin,0);
            // looking from dMin to dMax
            for(int d=dMin;d<=dMax;d++){
                // test if the window is in image2
                if(x+d>=win && x+d+win <= im2.width()){
                    // Calculate ncc and keep track of the best
                    float ncc = ccorrel(im1,x,y,im2,x+d,y);
                    if(ncc >= nccSeed && ncc >q.ncc){
                        q.d = d;
                        q.ncc = ncc;
                    }
                }
            }
            // add to seeds if ncc >= nccSeed then update if so
            if(q.ncc >= nccSeed){
                seeds(x,y) = true;
                disp(x,y) = q.d;
                Q.push(q);
        ...
        for(int i=0; i<4; i++) {
            ...
            if(0<=x-win && 0<=y-win &&
               x+win<im2.width() && y+win<im2.height() &&
                ...
                // ------------- TODO -------------
                Seed q = Seed(x,y,s.d,-1);
                // looking from -1 to +1
                for(int ii=-1;ii<=1;ii++){
                    float ncc = -1;
                    // test if the window is in image2
                    if(x+s.d+ii>=win && x+s.d+ii+win <= im2.width()){
                        // Calculate ncc and keep track of the best
                        ncc = ccorrel(im1,x,y,im2,x+s.d+ii,y);
                        if(ncc>q.ncc){
                            // restrict to dMin and dMax
                            q.d = min(s.d+ii,dMax);
                            q.d = max(s.d+ii,dMin);
                            q.ncc = ncc;
                        }
...
int main()
{
    ...
    //I1 = reduce(I1,256,256);
    //I2 = reduce(I2,256,256);
