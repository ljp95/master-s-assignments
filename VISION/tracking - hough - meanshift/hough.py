...
def hough_transform(h,w,R_table,orientation,threshold,alpha=30):
    #init
    ...
    #adding arbitrary size to not be out of boundary
    vote_mat = np.zeros((n+2*100,m+2*100))
    index = np.where(orientation>=threshold)
    for i,j in zip(index[0],index[1]):
        phi = orientation[i,j]
        for a,b in R_table[int(phi/alpha)]:
            vote_mat[i+100+a,j+100+b] += 1
    vote_mat = vote_mat[100:n+100,100:m+100]
    
    #blurring to avoid unlucky positions
    vote_mat = cv2.GaussianBlur(vote_mat,(3,3),cv2.BORDER_DEFAULT)
...
while(1):
    ...
    if ret == True:
        ...
        ''' Hough transform BEGIN '''
        #apply an update of the R_table
#        R_table = create_R_table(track_window,orientation,alpha)
        #compute thresholded orientation of gradient
        orientation,mask,module = local_or(frame,threshold)
        #apply hough transform
        track_window,vote_mat = hough_transform(h,w,R_table,orientation,threshold,alpha)
        ...
        #vote matrix
        cv2.imshow('Vote matrix',vote_mat)
        ''' show END '''
