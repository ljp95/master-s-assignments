...
''' functions BEGIN '''
def update_roi_hist(hsv,track_window,mask,roi_hist):
    (r,c,h,w) = track_window
    # set up the ROI for tracking in HSV space
    hsv_roi = hsv[c:c+w, r:r+h]
    # computation mask of the histogram : Pixels with S<30, V<20 or V>235 are ignored 
    mask = cv2.inRange(hsv_roi, np.array((0.,30.,20.)), np.array((180.,255.,235.)))
    # Marginal histogram of the Hue component with applied mask
    roi_hist = cv2.calcHist([hsv_roi],[0],mask,[180],[0,180])
    # Histogram values are normalised to [0,255]
    cv2.normalize(roi_hist,roi_hist,0,255,cv2.NORM_MINMAX)
    return roi_hist
...
#        roi_hist = update_roi_hist(hsv,track_window,mask,roi_hist)
        ''' mean-shift END '''
        ...
        if k == 27:
            ...
            cv2.imwrite('Hue_%04d.png'%cpt,hue)
